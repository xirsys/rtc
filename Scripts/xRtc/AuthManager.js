...
(function (exports) {
	...
	xrtc.AuthManager = xrtc.Class('AuthManager');
	...
	xrtc.AuthManager.include(xrtc.Ajax);
	xrtc.AuthManager.include(xrtc.EventDispatcher);
	xrtc.AuthManager.include({
		init: function () {
		...
		getToken: function (userData, callback) {
			...
			this.ajax(url, 'POST', data, this.proxy(handleTokenRequest, userData, callback));
		},

		getIceServers: function (token, callback) {
			var iceServers = this._iceServers[token];
			if (iceServers) {
				this._logger.info('getIceServers', iceServers);
				...
				if (iceServers && iceServers.iceServers && iceServers.iceServers.length > 0) {
					this._iceServers[token] = iceServers;
					...
					this.ajax(url, 'POST', data, this.proxy(handleIceServersRequest, token, callback));
				}
			}
		}
	});
	...
	function getTokenRequestParams(userData) {
		...
		this._logger.info('getTokenRequestParams', result);
	...
	function handleIceServersRequest(response, token, callback) {
		try {
			response = JSON.parse(response);
			this._logger.debug('getIceServers', response);

			if (!!response && !!response.E && response.E != '') {
				var error = new xrtc.CommonError('getIceServers', response.E);
				this._logger.error('getIceServers', error);
				this.trigger(xrtc.AuthManager.events.serverError, error);
				return;
			}
