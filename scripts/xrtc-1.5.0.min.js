...
b;else if(q.isStun(b)){var f=null,u;u=null;if(d)for(var w=0;w<d.length;w++){var s=d[w];0===s.url.indexOf("turn:")&&(u=-1!==s.url.indexOf("@")?s.url.split("@")[1]:s.url.split("turn:")[1])}u&&(f=b,f.candidate=b.candidate.replace(h,u),f.candidate=b.candidate.replace("typ srflx","typ relay"))}return f},filterSDP:function(b){var d=b;e===a.Connection.connectionTypes.server?d=b.replace(/a=candidate:.*((typ host)|(typ srflx)).*\r\n/g,""):e===a.Connection.connectionTypes.direct&&(d=b.replace(/a=candidate:.*typ relay.*\r\n/g,
...
0===f[0].indexOf("stun")?l={url:c(e,"/")}:0!==f[0].indexOf("turn")||-1===e.indexOf("transport=udp")&&-1!==e.indexOf("?transport")||(e=e.split("?"),l={url:c(e[0],"/"),credential:a,username:d});return d=l},l=0,f=a.length;l<f;l++){var x=e(a[l]);x&&d.push(x)}return d}function k(c){var f=this;c=h(c);l=new b.RTCPeerConnection(c&&0<c.length?{iceServers:c}:null,a.Connection.settings.peerConnectionOptions);p.info("initPeerConnection","PeerConnection created.");l.onicecandidate=r(function(a){a.candidate&&(p.debug("peerConnection.onIceCandidate",
a.candidate),a=JSON.parse(JSON.stringify(a.candidate)),a=H.filterCandidate(a),null!==a&&(I.push(a),M&&s.call(this)))});l.onstatechange=l.onsignalingstatechange=r(function(b){p.debug("onConnectionStateChange",b);this.trigger(a.Connection.events.stateChanged,{connection:this,state:this.getState()})});if(b.detectedBrowser===b.supportedBrowsers.firefox&&24>b.detectedBrowserVersion){var P=this.getState();N=d.setInterval(function(){var b=f.getState();b!=P&&(p.debug("setInterval -> xrtc.Connection.events.stateChanged",
b),P=b,f.trigger(a.Connection.events.stateChanged,{connection:f,state:P}))},500)}l.onicechange=l.oniceconnectionstatechange=r(function(b){b=x.call(f);p.debug("onIceStateChange",(new Date).getTime(),b);E&&(p.debug("onIceStateChange",(new Date).getTime(),"checkDisconnectedIceStateTimeout are clearing. ID = '"+E+"'"),d.clearTimeout(E),E=null,p.debug("onIceStateChange",(new Date).getTime(),"checkDisconnectedIceStateTimeout was cleared. ID = '"+E+"'"));"connected"===b?f.trigger(a.Connection.events.connectionEstablished,
...
f);G.push(b);f.trigger(a.Connection.events.dataChannelCreated,{connection:f,channel:b})};l.onaddstream=r(function(b){b=new a.Stream(b.stream);D.push(b);b={user:e,connection:this,stream:b};p.debug("addRemoteStream",b);this.trigger(a.Connection.events.remoteStreamAdded,b)});l.onclosedconnection=function(a){p.debug("peerConnection.onclosedconnection",a);m.call(f)};c=0;for(var n=B.length;c<n;c++)l.addStream(B[c].getStream());c=0;for(n=Q.length;c<n;c++)w.call(this,Q[c]);g()}e=c;l?g():n.call(this,r(k))}
function w(b){try{var c=null,e=null;b&&(b.constructor===d.String?c=b:(c=b.name,e=b.reliable));if(!c)throw"Data Channel name should be specified.";var f;e?f=l.createDataChannel(c,{reliable:e}):a.webrtc.supports.sctp?(f=l.createDataChannel(b.name,{reliable:!0}),f.binaryType="arraybuffer"):f=l.createDataChannel(b.name,{reliable:!1});var x=new a.DataChannel(f,this);G.push(x);this.trigger(a.Connection.events.dataChannelCreated,{connection:this,channel:x})}catch(r){c=new a.CommonError("createDataChannel",
"Can't create DataChannel.",r),p.error("createDataChannel",c),this.trigger(a.Connection.events.dataChannelCreationError,{connection:this,channelConfig:b,error:c})}}function s(){p.debug("sendIceCandidates",'Sending "'+I.length+'" ice candidates.');for(var b=0,c=I.length;b<c;b++){var d=I[b];J.sendIce(e.id,L,JSON.stringify(d));this.trigger(a.Connection.events.iceSent,{connection:this,iceCandidate:d})}I=[]}function n(a){"function"===typeof a&&(F?a(F):C.getIceServers(K,function(b){F=b;a(F)}))}function z(c){p.debug("Ice candidate was received.",
c);c=new b.RTCIceCandidate(JSON.parse(c.iceCandidate));l.addIceCandidate(c);this.trigger(a.Connection.events.iceAdded,{connection:this,iceCandidate:c})}function A(c){this.trigger(a.Connection.events.offerReceived,{connection:this,user:e,offerData:c});this.trigger(a.Connection.events.connectionOpening,{connection:this,user:e});p.debug("Offer was received.",c);F=c.iceServers;u.call(this,e,r(function(){p.debug("receiveOffer",c);H=new h.IceCandidateFilter(c.connectionType,F);var d=JSON.parse(c.offer),
d=new b.RTCSessionDescription(d);l.setRemoteDescription(d);l.createAnswer(r(function(d){b.detectedBrowser===b.supportedBrowsers.firefox&&(d.sdp=H.filterSDP(d.sdp));l.setLocalDescription(d);var f={answer:JSON.stringify(d),acceptData:c.acceptData};p.debug("sendAnswer",c,d);J.sendAnswer(e.id,L,f);this.trigger(a.Connection.events.answerSent,{connection:this,user:e,answerData:f});this.trigger(a.Connection.events.connectionEstablishing,{connection:this,user:e});M=!0;s.call(this)}),r(function(b){b=new a.CommonError("sendAnswer",
"Cannot create WebRTC answer",b);p.error("sendAnswer",b);this.trigger(a.Connection.events.createAnswerError,{connection:this,error:b})}),a.Connection.settings.answerOptions)}))}function t(c){p.debug("Answer was received.",c);M=!0;s.call(this);c=JSON.parse(c.answer);c=new b.RTCSessionDescription(c);l.setRemoteDescription(c);this.trigger(a.Connection.events.answerReceived,{connection:this,user:e,answerData:{answer:c}});this.trigger(a.Connection.events.connectionEstablishing,{connection:this,user:e})}
function y(){m.call(this)}function m(){b.detectedBrowser===b.supportedBrowsers.firefox&&N&&(d.clearInterval(N),N=null);if(l){l.onicecandidate=null;l.close();l=null;I=[];F=null;O=M=!1;var c={user:e,connection:this};e=null;this.trigger(a.Connection.events.connectionClosed,c)}}function x(){return l&&(l.iceConnectionState||l.iceState)||"notinitialized"}var r=a.Class.proxy(this),p=new a.Logger(this.className),K=g,C=q||new xRtc.AuthManager,B=[],D=[],G=[],Q=[],l=null,N=null,E=null,J=k,H=null,F=null,M=!1,
I=[],L=f,O=!1;(function(){var b=a.HandshakeController.events;J.on(b.receiveIce,r(z)).on(b.receiveOffer,r(A)).on(b.receiveAnswer,r(t)).on(b.receiveBye,r(y))}).call(this);a.Class.extend(this,a.EventDispatcher,{_logger:p,getId:function(){return L},getRemoteUser:function(){return e},_open:function(d){O=!0;var f=this,x={};a.Class.extend(x,a.Connection.settings.offerOptions);d&&d.offer&&a.Class.extend(x,d.offer);f.trigger(a.Connection.events.connectionOpening,{connection:f,user:e});u.call(f,e,function(){H=
new h.IceCandidateFilter(d&&d.connectionType||null,F);l.createOffer(r(function(d){if(l){b.detectedBrowser===b.supportedBrowsers.firefox&&(d.sdp=H.filterSDP(d.sdp));p.debug("onCreateOfferSuccess",d);l.setLocalDescription(d);b.detectedBrowser===b.supportedBrowsers.firefox&&21>=b.detectedBrowserVersion&&(d.sdp=-1==d.sdp.indexOf("a=crypto")?d.sdp.replace(/c=IN/g,"a=crypto:1 AES_CM_128_HMAC_SHA1_80 inline:FakeFakeFakeFakeFakeFakeFakeFakeFakeFake\r\nc=IN"):d.sdp);var x={offer:JSON.stringify(d),connectionData:c,
connectionType:H.getType(),iceServers:F};p.debug("sendOffer",e.id,d);J.sendOffer(e.id,L,x);f.trigger(a.Connection.events.offerSent,{connection:this,user:e,offerData:x})}}),r(function(b){b=new a.CommonError("startSession","Cannot create WebRTC offer",b);p.error("onCreateOfferError",b);f.trigger(a.Connection.events.createOfferError,{connection:f,error:b})}),x)})},close:function(a){J&&e&&J.sendBye(e.id,L,a);m.call(this)},addStream:function(b){O&&v("addStream");B.push(b);b={connection:this,stream:b,user:{id:K.name,
name:K.name}};p.debug("addLocalStream",b);this.trigger(a.Connection.events.localStreamAdded,b)},createDataChannel:function(a,b){O&&v("createDataChannel");Q.push({name:a,config:b})},getData:function(){return c},getState:function(){var a=0<B.length;return{notinitialized:a?"ready":"not-ready","new":a?"ready":"not-ready",opening:"connecting",active:"connected",closing:"disconnecting",closed:a?"ready":"not-ready",stable:"connected","have-local-offer":"ready","have-remote-offer":"connecting"}[l&&(l.signalingState||
l.readyState)||"notinitialized"]},getLocalStreams:function(){return B.map(function(a){return a})},getRemoteStreams:function(){return D.map(function(a){return a})},getDataChannels:function(){return G.map(function(a){return a})}})});a.Connection.extend({events:{connectionOpening:"connectionopening",connectionEstablishing:"connectionestablishing",connectionEstablished:"connectionestablished",connectionClosed:"connectionclosed",localStreamAdded:"localstreamadded",remoteStreamAdded:"remotestreamadded",
dataChannelCreated:"datachannelcreated",dataChannelCreationError:"datachannelcreationerror",stateChanged:"statechanged",createOfferError:"createoffererror",offerSent:"offersent",offerReceived:"offerreceived",createAnswerError:"createanswererror",answerSent:"answersent",answerReceived:"answerreceived",iceSent:"icesent",iceAdded:"iceadded"},connectionTypes:{"default":"default",direct:"direct",server:"server"},settings:{offerOptions:{optional:[],mandatory:{OfferToReceiveAudio:!0,OfferToReceiveVideo:!0}},
answerOptions:{optional:[],mandatory:{OfferToReceiveAudio:!0,OfferToReceiveVideo:!0}},peerConnectionOptions:{optional:[{RtpDataChannels:!a.webrtc.supports.sctp},{DtlsSrtpKeyAgreement:!0}]}}});b.RTCPeerConnection.prototype&&!b.RTCPeerConnection.prototype.getLocalStreams&&a.Class.extend(b.RTCPeerConnection.prototype,{getLocalStreams:function(){return this.localStreams},getRemoteStreams:function(){return this.remoteStreams}});b.MediaStream.prototype.getVideoTracks||(b.detectedBrowser===b.supportedBrowsers.firefox?
a.Class.extend(b.MediaStream.prototype,{getVideoTracks:function(){return[]},getAudioTracks:function(){return[]}}):a.Class.extend(b.MediaStream.prototype,{getVideoTracks:function(){return this.videoTracks},getAudioTracks:function(){return this.audioTracks}}))})(window);(function(d){"undefined"===typeof d.xRtc&&(d.xRtc={});var a=d.xRtc;a.Class(a,"HandshakeController",function(){var d=new a.Logger(this.className);a.Class.extend(this,a.EventDispatcher,{_logger:d,sendIce:function(b,d,g){this.trigger(a.HandshakeController.events.sendIce,b,d,g)},sendOffer:function(b,d,g){this.trigger(a.HandshakeController.events.sendOffer,b,d,g)},sendAnswer:function(b,d,g){this.trigger(a.HandshakeController.events.sendAnswer,b,d,g)},sendBye:function(b,d,g){this.trigger(a.HandshakeController.events.sendBye,
...
d);n.error("parseServerMessage",e,b);this.trigger(m.messageFormatError,{error:e})}return c}function w(a){return d.setInterval(function(){b.call(this,{})},a)}var s=a.Class.proxy(this),n=new a.Logger(this.className),z=null,A=null,t=h?h.pingInterval:5E3,y=null,m=a.ServerConnector.events;a.Class.extend(this,a.EventDispatcher,a.Ajax,{_logger:n,connect:function(b){A=b;b=s(g,b);this.ajax(a.ServerConnector.settings.URL,"POST","",s(f,b))},disconnect:function(){z?(z.close(),A=z=null,n.info("disconnect","Connection with WS has been broken")):
...
break}return b}function u(a){for(var b=null,c=0,d=B.length;c<d;c++)if(B[c].getId()===a){b=B[c];break}return b}function w(a,b){for(var c=-1,d=0,e=a.length;d<e;d++)if(a[d].getId()===b){c=d;break}return c}function s(a,b){for(var c=-1,d=0,e=a.length;d<e;d++)a[d].id===b&&(c=d);return c}var n=a.Class.proxy(this),z=new a.Logger(this.className),A=a.HandshakeController.events,t=a.ServerConnector.events,y=[],m=!1,x={},r={},p=null,K=b||new xRtc.AuthManager,C=f||new a.ServerConnector,B=[],D={},G={decline:"decline"};
a.Class.extend(r,a.Room.settings.info);"string"===typeof d?r.name=d:a.Class.extend(r,d);a.Class.extend(this,a.EventDispatcher,{_logger:z,enter:function(b,c){var d="",e="";if(!b)throw new a.CommonError("enter","User credentials should be specified.");"string"===typeof b?d=b:(d=b.username,e=b.password);g.call(this);a.Class.extend(x,a.Room.settings.enterOptions);c&&a.Class.extend(x,c);p={domain:r.domain,application:r.application,room:r.name,name:d,password:e};K.getToken(p,function(a){r.user={id:null,
name:d};C.connect(a)})},leave:function(){C.on(a.ServerConnector.events.connectionClose,function(){m&&(C.off(t.connectionOpen).off(t.connectionClose).off(t.tokenInvalid).off(t.usersUpdated).off(t.userConnected).off(t.userDisconnected).off(t.receiveOffer).off(t.receiveBye).off(t.receiveAnswer).off(t.receiveIce),m=!1);x={};p=null;r.user=null;y=[];B=[];D={}});C.disconnect()},connect:function(b,c){if(!r.user)throw new a.CommonError("connect","Need to enter the room before you connect someone.");var d=
v(b);if(null==d)d=a.CommonError("connect","Target user not found."),this.trigger(a.Room.events.error,{userId:b,error:d});else{var e=c&&c.data?c.data:null,f=c&&c.id?c.id:a.utils.newGuid();q.call(this,f,p,d,e,function(a){a=a.connection;c&&"auto"===c.createDataChannel&&a.createDataChannel("autoDataChannel");a._open(c)})}},getInfo:function(){return r},getConnections:function(){return B.map(function(a){return a})},getUsers:function(){return y.map(function(a){return a})}})});a.Room.extend({events:{enter:"enter",
