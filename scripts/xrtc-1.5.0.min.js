...
f);G.push(b);f.trigger(a.Connection.events.dataChannelCreated,{connection:f,channel:b})};m.onaddstream=v(function(b){b=new a.Stream(b.stream);D.push(b);b={user:e,connection:this,stream:b};q.debug("addRemoteStream",b);this.trigger(a.Connection.events.remoteStreamAdded,b)});m.onclosedconnection=function(a){q.debug("peerConnection.onclosedconnection",a);l.call(f)};c=0;for(var p=B.length;c<p;c++)m.addStream(B[c].getStream());c=0;for(p=Q.length;c<p;c++)x.call(this,Q[c]);g()}e=c;m?g():n.call(this,v(k))}
...
y.onopen=s(e);y.onclose=s(k);y.onerror=s(p);y.onmessage=s(c)}function e(a){a={event:a};n.debug("open",a);u&&(z=x.call(this,u));this.trigger(l.connectionOpen,a)}function k(a){z&&(d.clearInterval(z),z=null);a={event:a};n.debug("close",a);this.trigger(l.connectionClose,a);y=null}function p(b){b=new a.CommonError("onerror","WebSocket has got an error",b);n.error("error",b);this.trigger(l.connectionError,{error:b})}function c(a){var b={message:a};n.debug("message",b);this.trigger(l.message,b);if(r(a))if(a=
...
d);n.error("parseServerMessage",e,b);this.trigger(l.messageFormatError,{error:e})}return c}function x(a){return d.setInterval(function(){b.call(this,{})},a)}var s=a.Class.proxy(this),n=new a.Logger(this.className),y=null,A=null,u=h?h.pingInterval:5E3,z=null,l=a.ServerConnector.events;a.Class.extend(this,a.EventDispatcher,a.Ajax,{_logger:n,connect:function(b){A=b;b=s(f,b);this.ajax(a.ServerConnector.settings.URL,"POST","",s(g,b))},disconnect:function(){y?(y.close(),A=y=null,n.info("disconnect","Connection with WS has been broken")):
...
null,c=0,d=B.length;c<d;c++)if(B[c].getId()===a){b=B[c];break}return b}function x(a,b){for(var c=-1,d=0,e=a.length;d<e;d++)if(a[d].getId()===b){c=d;break}return c}function s(a,b){for(var c=-1,d=0,e=a.length;d<e;d++)a[d].id===b&&(c=d);return c}var n=a.Class.proxy(this),y=new a.Logger(this.className),A=a.HandshakeController.events,u=a.ServerConnector.events,z=[],l=!1,t={},v={},q=null,K=b||new xRtc.AuthManager,C=g||new a.ServerConnector,B=[],D={},G={decline:"decline"};a.Class.extend(v,a.Room.settings.info);
"string"===typeof d?v.name=d:a.Class.extend(v,d);a.Class.extend(this,a.EventDispatcher,{_logger:y,enter:function(b,c){var d="",e="";if(!b)throw new a.CommonError("enter","User credentials should be specified.");"string"===typeof b?d=b:(d=b.username,e=b.password);f.call(this);a.Class.extend(t,a.Room.settings.enterOptions);c&&a.Class.extend(t,c);q={domain:v.domain,application:v.application,room:v.name,name:d,password:e};K.getToken(q,function(a){v.user={id:null,name:d};C.connect(a)})},leave:function(){C.on(a.ServerConnector.events.connectionClose,
function(){l&&(C.off(u.connectionOpen).off(u.connectionClose).off(u.tokenInvalid).off(u.usersUpdated).off(u.userConnected).off(u.userDisconnected).off(u.receiveOffer).off(u.receiveBye).off(u.receiveAnswer).off(u.receiveIce),l=!1);t={};q=null;v.user=null;z=[];B=[];D={}});C.disconnect()},connect:function(b,c){if(!v.user)throw new a.CommonError("connect","Need to enter the room before you connect someone.");var d=r(b);if(null==d)d=a.CommonError("connect","Target user not found."),this.trigger(a.Room.events.error,
{userId:b,error:d});else{var e=c&&c.data?c.data:null,f=c&&c.id?c.id:a.utils.newGuid();p.call(this,f,q,d,e,function(a){a=a.connection;c&&"auto"===c.createDataChannel&&a.createDataChannel("autoDataChannel");a._open(c)})}},getInfo:function(){return v},getConnections:function(){return B.map(function(a){return a})},getUsers:function(){return z.map(function(a){return a})}})});a.Room.extend({events:{enter:"enter",leave:"leave",incomingConnection:"incomingconnection",connectionCreated:"connectioncreated",
