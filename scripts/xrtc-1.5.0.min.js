...
var d=!1,b=!1,g=!1,f;try{f=new a.webrtc.RTCPeerConnection(null,{optional:[{RtpDataChannels:!0}]});d=!0;try{f.createDataChannel("_XRTCTEST",{reliable:!1});var b=!0,e=new a.webrtc.RTCPeerConnection(null,{});try{g=e.createDataChannel("_XRTCRELIABLETEST",{reliable:!0}).reliable}catch(h){}e.close()}catch(w){}}catch(c){}f&&f.close();return{media:d,data:b,sctp:g}}();a.blobSerializer={pack:BinaryPack.pack,unpack:BinaryPack.unpack};xRtc.utils={newGuid:function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,
...
c)},a.AuthManager.settings.unsuccessfulRequestRepeatTimeout)}}function g(b,d,c){var f=this;try{if(b=JSON.parse(b),e.debug("getIceServers",b),b&&b.e&&""!=b.e){var g=new a.CommonError("getIceServers",b.e);e.error("getIceServers",g);f.trigger(a.AuthManager.events.serverError,g)}else{var k=b.d.iceServers?b.d.iceServers.map(function(a){var c={};a.url&&(c.url=a.url);a.credential&&(c.credential=a.credential);a.username&&(c.username=a.username);return c}):[];e.info("getIceServers",k);"function"===typeof c&&
...
delete f[c.messageId]})}}var x=e.Class.proxy(this),s=new e.Logger(this.className),v=e.DataChannel.events,q=10,m=new a(new b(new a(new g(new f(h,100))),16300)),r={};h.onopen=x(function(a){a={event:a};s.debug("open",a);this.trigger(v.open,a)});h.onmessage=x(function(a){var b=this;s.debug("message",a.data);var e=a.data.constructor;e===d.ArrayBuffer?c.call(b,a.data):e===d.Blob&&k(a.data,function(a){c.call(b,a)})});h.onclose=x(function(a){a={event:a};s.debug("close",a);this.trigger(v.close,a)});h.onerror=
x(function(a){a=new e.CommonError("onerror","DataChannel error.",a);s.error("error",a);this.trigger(v.error,a)});h.ondatachannel=x(function(a){a={event:a};s.debug("datachannel",a);this.trigger(v.dataChannel,a)});e.Class.extend(this,e.EventDispatcher,{_logger:s,getId:function(){return w.getId()+this.getName()},getRemoteUser:function(){return w.getRemoteUser()},getConnection:function(){return w},getName:function(){return h.label},getState:function(){return h.readyState.toLowerCase()},send:function(a,
b,c,d){var f=this,h=f.getState();h!==e.DataChannel.states.open&&(h=new e.CommonError("send",'DataChannel should be opened before sending some data. Current channel state is "'+h+'"'),s.error("error",h),f.trigger(v.error,h));s.info("send",a);m.send(a,function(){var c={data:a};"function"===typeof b&&b(c);f.trigger(v.sentMessage,c)},function(a){a=new e.CommonError("send","DataChannel error.",a);s.error("error",a);"function"===typeof c&&c(a);f.trigger(v.error,a)},function(a){0!==a.count%q&&a.count!==
...
b;else if(k.isStun(b)){var g=null,s;s=null;if(d)for(var v=0;v<d.length;v++){var q=d[v];0===q.url.indexOf("turn:")&&(s=-1!==q.url.indexOf("@")?q.url.split("@")[1]:q.url.split("turn:")[1])}s&&(g=b,g.candidate=b.candidate.replace(h,s),g.candidate=b.candidate.replace("typ srflx","typ relay"))}return g},filterSDP:function(b){var d=b;e===a.Connection.connectionTypes.server?d=b.replace(/a=candidate:.*((typ host)|(typ srflx)).*\r\n/g,""):e===a.Connection.connectionTypes.direct&&(d=b.replace(/a=candidate:.*typ relay.*\r\n/g,
...
f);G.push(b);f.trigger(a.Connection.events.dataChannelCreated,{connection:f,channel:b})};n.onaddstream=u(function(b){b=new a.Stream(b.stream);C.push(b);b={user:e,connection:this,stream:b};p.debug("addRemoteStream",b);this.trigger(a.Connection.events.remoteStreamAdded,b)});n.onclosedconnection=function(a){p.debug("peerConnection.onclosedconnection",a);l.call(f)};c=0;for(var m=A.length;c<m;c++)n.addStream(A[c].getStream());c=0;for(m=Q.length;c<m;c++)v.call(this,Q[c]);g()}e=c;n?g():m.call(this,u(h))}
...
r.onopen=q(e);r.onclose=q(h);r.onerror=q(w);r.onmessage=q(c)}function e(a){a={event:a};m.debug("open",a);t&&(y=v.call(this,t));this.trigger(l.connectionOpen,a)}function h(a){y&&(d.clearInterval(y),y=null);a={event:a};m.debug("close",a);this.trigger(l.connectionClose,a);r=null}function w(b){b=new a.CommonError("onerror","WebSocket has got an error",b);m.error("error",b);this.trigger(l.connectionError,{error:b})}function c(a){var b={message:a};m.debug("message",b);this.trigger(l.message,b);if(x(a))if(a=
...
d);m.error("parseServerMessage",e,b);this.trigger(l.messageFormatError,{error:e})}return c}function v(a){return d.setInterval(function(){b.call(this,{})},a)}var q=a.Class.proxy(this),m=new a.Logger(this.className),r=null,z=null,t=k?k.pingInterval:5E3,y=null,l=a.ServerConnector.events;a.Class.extend(this,a.EventDispatcher,a.Ajax,{_logger:m,connect:function(b){z=b;b=q(f,b);this.ajax(a.ServerConnector.settings.URL,"POST","",q(g,b))},disconnect:function(){r?(r.close(),z=r=null,m.info("disconnect","Connection with WS has been broken")):
