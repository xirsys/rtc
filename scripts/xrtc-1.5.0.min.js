...
a);f.error("error",a);this.trigger(e.error,a)});a.ondatachannel=d(function(b){b={event:b};f.debug("datachannel",b);this.trigger(e.dataChannel,b)});b.Class.extend(this,b.EventDispatcher,{_logger:f,getId:function(){return c.getId()+this.getName()},getRemoteUser:function(){return c.getRemoteUser()},getConnection:function(){return c},getName:function(){return a.label},getState:function(){return a.readyState.toLowerCase()},send:function(c){f.info("send",arguments);try{a.send(c)}catch(d){var n=new b.CommonError("onerror",
'DataChannel sending error. Channel state is "'+this.getState()+'"',d);f.error("error",n);this.trigger(e.error,n)}this.trigger(e.sentMessage,{data:c})}})});b.DataChannel.extend({events:{open:"open",sentMessage:"sentMessage",receivedMessage:"receivedMessage",close:"close",error:"error",dataChannel:"datachannel"},states:{connecting:"connecting",open:"open",closing:"closing",closed:"closed"}})})(window);xRtc.handshakeController={};
...
k.debug("getWebSocketURL",a),a&&a.e&&""!=a.e){var e=new b.CommonError("getWebSocketURL","Error occured while getting the URL of WebSockets",a.e);k.error("getWebSocketURL",e);this.trigger(g.serverError,{error:e})}else{var f=a.d.value;k.info("getWebSocketURL",f);"function"===typeof c&&c(f)}}catch(n){this.ajax(b.ServerConnector.settings.URL,"POST","",s(d,c))}}function f(b,a){y=new WebSocket(b+"/ws/"+encodeURIComponent(a));y.onopen=s(e);y.onclose=s(p);y.onerror=s(t);y.onmessage=s(n)}function e(b){b={event:b};
k.debug("open",b);r&&(u=v.call(this,r));this.trigger(g.connectionOpen,b)}function p(b){u&&(a.clearInterval(u),u=null);b={event:b};k.debug("close",b);this.trigger(g.connectionClose,b);y=null}function t(a){a=new b.CommonError("onerror","WebSocket has got an error",a);k.error("error",a);this.trigger(g.connectionError,{error:a})}function n(b){var a={message:b};k.debug("message",a);this.trigger(g.message,a);if(w(b))if(b=x(b),a=b.type,a==g.usersUpdated||a==g.userConnected||a==g.userDisconnected)if(a==g.usersUpdated){for(var a=
...
a==g.receiveBye&&this.trigger(g.receiveBye,{senderId:b.userid,connectionId:b.message.data.connectionId,byeData:b.message.data.byeData})}function w(b){var a=!0;'"Token invalid"'===b.data&&(a=!1,this.trigger(g.tokenInvalid,{token:z}));return a}function x(a){var c;try{c=JSON.parse(a.data)}catch(e){c=null;var d=new b.CommonError("parseServerMessage","Message format error",e);k.error("parseServerMessage",d,a);this.trigger(g.messageFormatError,{error:d})}return c}function v(b){return a.setInterval(function(){c.call(this,
{})},b)}var s=b.Class.proxy(this),k=new b.Logger(this.className),y=null,z=null,r=l?l.pingInterval:5E3,u=null,g=b.ServerConnector.events;b.Class.extend(this,b.EventDispatcher,b.Ajax,{_logger:k,connect:function(a){z=a;a=s(f,a);this.ajax(b.ServerConnector.settings.URL,"POST","",s(d,a))},disconnect:function(){y?(y.close(),z=y=null,k.info("disconnect","Connection with WS has been broken")):k.debug("disconnect","Connection with WS has not been established yet")},sendOffer:function(b,a,e){c({eventName:g.receiveOffer,
...
function p(b){function a(b,c){var e=b;b[b.length-1]===c&&(e=b.substring(0,b.length-1));return e}for(var e=[],h=function(b){var e;if(c.detectedBrowser==c.supportedBrowsers.chrome){var h=b.url;e=b.username;b=b.credential;var d=null,f=h.split(":");0===f[0].indexOf("stun")?d={url:a(h,"/")}:0===f[0].indexOf("turn")&&(28>c.detectedBrowserVersion?(h=h.split("turn:"),d={url:"turn:"+e+"@"+h[1],credential:b}):d={url:a(h,"/"),credential:b,username:e})}else h=b.url,e=b.username,b=b.credential,d=null,f=h.split(":"),
0===f[0].indexOf("stun")?d={url:a(h,"/")}:0!==f[0].indexOf("turn")||-1===h.indexOf("transport=udp")&&-1!==h.indexOf("?transport")||(h=h.split("?"),d={url:a(h[0],"/"),credential:b,username:e});return e=d},d=0,f=b.length;d<f;d++){var I=h(b[d]);I&&e.push(I)}return e}function n(d){var f=this;d=p(d);h=new c.RTCPeerConnection(d&&0<d.length?{iceServers:d}:null,b.Connection.settings.peerConnectionOptions);m.info("initPeerConnection","PeerConnection created.");h.onicecandidate=q(function(b){b.candidate&&(m.debug("peerConnection.onIceCandidate",
b.candidate),b=JSON.parse(JSON.stringify(b.candidate)),b=J.filterCandidate(b),null!==b&&(K.push(b),N&&s.call(this)))});h.onstatechange=h.onsignalingstatechange=q(function(a){m.debug("onConnectionStateChange",a);this.trigger(b.Connection.events.stateChanged,{connection:this,state:this.getState()})});if(c.detectedBrowser===c.supportedBrowsers.firefox&&24>c.detectedBrowserVersion){var I=this.getState();O=a.setInterval(function(){var a=f.getState();a!=I&&(m.debug("setInterval -> xrtc.Connection.events.stateChanged",
a),I=a,f.trigger(b.Connection.events.stateChanged,{connection:f,state:I}))},500)}h.onicechange=h.oniceconnectionstatechange=q(function(c){c=F.call(f);m.debug("onIceStateChange",(new Date).getTime(),c);D&&(m.debug("onIceStateChange",(new Date).getTime(),"checkDisconnectedIceStateTimeout are clearing. ID = '"+D+"'"),a.clearTimeout(D),D=null,m.debug("onIceStateChange",(new Date).getTime(),"checkDisconnectedIceStateTimeout was cleared. ID = '"+D+"'"));"connected"===c?f.trigger(b.Connection.events.connectionEstablished,
{connection:f,user:e}):"disconnected"===c&&(m.debug("onIceStateChange",(new Date).getTime(),"checkDisconnectedIceStateTimeout(10sec.) was started."),D=a.setTimeout(function(){m.debug("onIceStateChange",(new Date).getTime(),"ice state equals 'disconnected' so closePeerConnection was called. Timeout is 10sec and it is expired.");g.call(f);a.clearInterval(D);D=null},1E4),m.debug("onIceStateChange",(new Date).getTime(),"checkDisconnectedIceStateTimeout ID ='"+D+"'"))});h.ondatachannel=function(a){a=new b.DataChannel(a.channel,
f);G.push(a);f.trigger(b.Connection.events.dataChannelCreated,{connection:f,channel:a})};h.onaddstream=q(function(a){a=new b.Stream(a.stream);C.push(a);a={user:e,connection:this,stream:a};m.debug("addRemoteStream",a);this.trigger(b.Connection.events.remoteStreamAdded,a)});h.onclosedconnection=function(b){m.debug("peerConnection.onclosedconnection",b);g.call(f)};d=0;for(var k=A.length;d<k;d++)h.addStream(A[d].getStream());d=0;for(k=Q.length;d<k;d++)v.call(this,Q[d]);l()}e=d;h?l():k.call(this,q(n))}
function v(a){try{var c;b.webrtc.supports.sctp?(c=h.createDataChannel(a.name,{reliable:!0}),c.binaryType="arraybuffer"):c=h.createDataChannel(a.name,{reliable:!1});var e=new b.DataChannel(c,this);G.push(e);this.trigger(b.Connection.events.dataChannelCreated,{connection:this,channel:e})}catch(d){c=new b.CommonError("createDataChannel","Can't create DataChannel.",d),m.error("createDataChannel",c),this.trigger(b.Connection.events.dataChannelCreationError,{connection:this,channelConfig:a,error:c})}}function s(){m.debug("sendIceCandidates",
'Sending "'+K.length+'" ice candidates.');for(var a=0,c=K.length;a<c;a++){var d=K[a];L.sendIce(e.id,M,JSON.stringify(d));this.trigger(b.Connection.events.iceSent,{connection:this,iceCandidate:d})}K=[]}function k(b){"function"===typeof b&&(E?b(E):B.getIceServers(H,function(a){E=a;b(E)}))}function y(a){m.debug("Ice candidate was received.",a);a=new c.RTCIceCandidate(JSON.parse(a.iceCandidate));h.addIceCandidate(a);this.trigger(b.Connection.events.iceAdded,{connection:this,iceCandidate:a})}function z(a){this.trigger(b.Connection.events.offerReceived,
{connection:this,user:e,offerData:a});this.trigger(b.Connection.events.connectionOpening,{connection:this,user:e});m.debug("Offer was received.",a);E=a.iceServers;x.call(this,e,q(function(){m.debug("receiveOffer",a);J=new l.IceCandidateFilter(a.connectionType,E);var d=JSON.parse(a.offer),d=new c.RTCSessionDescription(d);h.setRemoteDescription(d);h.createAnswer(q(function(d){c.detectedBrowser===c.supportedBrowsers.firefox&&(d.sdp=J.filterSDP(d.sdp));h.setLocalDescription(d);var f={answer:JSON.stringify(d),
...
a);N=!0;s.call(this);a=JSON.parse(a.answer);a=new c.RTCSessionDescription(a);h.setRemoteDescription(a);this.trigger(b.Connection.events.answerReceived,{connection:this,user:e,answerData:{answer:a}});this.trigger(b.Connection.events.connectionEstablishing,{connection:this,user:e})}function u(){g.call(this)}function g(){c.detectedBrowser===c.supportedBrowsers.firefox&&O&&(a.clearInterval(O),O=null);if(h){h.onicecandidate=null;h.close();h=null;K=[];E=null;P=N=!1;var d={user:e,connection:this};e=null;
this.trigger(b.Connection.events.connectionClosed,d)}}function F(){return h&&(h.iceConnectionState||h.iceState)||"notinitialized"}var q=b.Class.proxy(this),m=new b.Logger(this.className),H=f,B=t||new xRtc.AuthManager,A=[],C=[],G=[],Q=[],h=null,O=null,D=null,L=p,J=null,E=null,N=!1,K=[],M=d,P=!1;(function(){var a=b.HandshakeController.events;L.on(a.receiveIce,q(y)).on(a.receiveOffer,q(z)).on(a.receiveAnswer,q(r)).on(a.receiveBye,q(u))}).call(this);b.Class.extend(this,b.EventDispatcher,{_logger:m,getId:function(){return M},
getRemoteUser:function(){return e},_open:function(a){P=!0;var d=this,f={};b.Class.extend(f,b.Connection.settings.offerOptions);a&&a.offer&&b.Class.extend(f,a.offer);d.trigger(b.Connection.events.connectionOpening,{connection:d,user:e});x.call(d,e,function(){J=new l.IceCandidateFilter(a&&a.connectionType||null,E);h.createOffer(q(function(a){if(h){c.detectedBrowser===c.supportedBrowsers.firefox&&(a.sdp=J.filterSDP(a.sdp));m.debug("onCreateOfferSuccess",a);h.setLocalDescription(a);c.detectedBrowser===
...
{connectionId:c.connectionId,answer:c.answer})):B.sendBye(c.senderId,c.connectionId,{type:G.decline,data:"Remote connection not found."})}}}).on(r.receiveIce,function(b){if(b.senderId&&b.connectionId){var a=C[b.connectionId];a&&a.userId===b.senderId&&a.hc.trigger(z.receiveIce,{iceCandidate:b.iceCandidate,connectionId:b.connectionId})}}),g=!0)}function e(a){function c(b){t.call(e,a.connectionId,m,w(a.senderId),a.connectionData,function(c){c.handshakeController.trigger(z.receiveOffer,{offer:a.offer,
iceServers:a.iceServers,connectionType:a.connectionType,connectionId:a.connectionId,connectionData:a.connectionData,acceptData:b})})}function d(b){B.sendBye(a.senderId,a.connectionId,{type:G.decline,data:b})}if(a.senderId&&a.connectionId){var e=this,f={user:w(a.senderId),connectionId:a.connectionId,data:a.connectionData};F.autoReply||(f.accept=k(c),f.decline=k(d));C[a.connectionId]={userId:a.senderId,hc:null};this.trigger(b.Room.events.incomingConnection,f);F.autoReply&&c.call(e)}}function p(a){if(a.senderId&&
a.connectionId){var c=w(a.senderId);if(c){var d=C[a.connectionId];if(d&&d.userId===c.id){if(a.byeData&&a.byeData.type===G.decline||!d.hc){var e=x(a.connectionId);e&&this.trigger(b.Room.events.connectionDeclined,{user:c,connection:e,data:a.byeData})}d.hc&&d.hc.trigger(z.receiveBye)}}}}function t(a,c,d,e,f){var g=new b.HandshakeController;c=new xRtc.Connection(a,c,d,g,H,e);C[a]?C[a].hc=g:C[a]={userId:d.id,hc:g};g.on(z.sendOffer,k(function(a,b,c){B.sendOffer(a,b,c)})).on(z.sendAnswer,k(function(a,b,
c){B.sendAnswer(a,b,c)})).on(z.sendIce,k(function(a,b,c){B.sendIce(a,b,c)})).on(z.sendBye,k(function(a,b,c){B.sendBye(a,b,c)}));c.on(b.Connection.events.connectionClosed,function(){A.splice(v(A,a),1);delete C[a]});A.push(c);this.trigger(b.Room.events.connectionCreated,{user:d,connection:c});"function"===typeof f&&f({connection:c,handshakeController:g})}function n(a,b){var c=0;a.name<b.name?c=-1:a.name>b.name&&(c=1);return c}function w(a){for(var b=null,c=0,d=u.length;c<d;c++)if(u[c].id===a){b=u[c];
break}return b}function x(a){for(var b=null,c=0,d=A.length;c<d;c++)if(A[c].getId()===a){b=A[c];break}return b}function v(a,b){for(var c=-1,d=0,e=a.length;d<e;d++)if(a[d].getId()===b){c=d;break}return c}function s(a,b){for(var c=-1,d=0,e=a.length;d<e;d++)a[d].id===b&&(c=d);return c}var k=b.Class.proxy(this),y=new b.Logger(this.className),z=b.HandshakeController.events,r=b.ServerConnector.events,u=[],g=!1,F={},q={},m=null,H=c||new xRtc.AuthManager,B=d||new b.ServerConnector,A=[],C={},G={decline:"decline"};
b.Class.extend(q,b.Room.settings.info);"string"===typeof a?q.name=a:b.Class.extend(q,a);b.Class.extend(this,b.EventDispatcher,{_logger:y,enter:function(a,c){var d="",e="";if(!a)throw new b.CommonError("enter","User credentials should be specified.");"string"===typeof a?d=a:(d=a.username,e=a.password);f.call(this);b.Class.extend(F,b.Room.settings.enterOptions);c&&b.Class.extend(F,c);m={domain:q.domain,application:q.application,room:q.name,name:d,password:e};H.getToken(m,function(a){q.user={id:null,
name:d};B.connect(a)})},leave:function(){B.on(b.ServerConnector.events.connectionClose,function(){g&&(B.off(r.connectionOpen).off(r.connectionClose).off(r.tokenInvalid).off(r.usersUpdated).off(r.userConnected).off(r.userDisconnected).off(r.receiveOffer).off(r.receiveBye).off(r.receiveAnswer).off(r.receiveIce),g=!1);F={};m=null;q.user=null;u=[];A=[];C={}});B.disconnect()},connect:function(a,c){if(!q.user)throw new b.CommonError("connect","Need to enter the room before you connect someone.");var d=
w(a);if(null==d)d=b.CommonError("connect","Target user not found."),this.trigger(b.Room.events.error,{userId:a,error:d});else{var e=c&&c.data?c.data:null,f=c&&c.id?c.id:b.utils.newGuid();t.call(this,f,m,d,e,function(a){a=a.connection;c&&"auto"===c.createDataChannel&&a.createDataChannel("autoDataChannel");a._open(c)})}},getInfo:function(){return q},getConnections:function(){return A.map(function(a){return a})},getUsers:function(){return u.map(function(a){return a})}})});b.Room.extend({events:{enter:"enter",
leave:"leave",incomingConnection:"incomingconnection",connectionCreated:"connectioncreated",connectionAccepted:"connectionaccepted",connectionDeclined:"connectiondeclined",usersUpdated:"usersupdated",userConnected:"userconnected",userDisconnected:"userdisconnected",tokenInvalid:"tokeninvalid",error:"error"},settings:{info:{domain:a.document.domain,application:"default",name:"default"},enterOptions:{autoReply:!0}}})})(window);xRtc.userMedia={};
