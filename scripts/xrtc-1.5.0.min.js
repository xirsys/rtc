...
(function(a){"undefined"===typeof a.xRtc&&(a.xRtc={});var b=a.xRtc;b.Class(b,"ServerConnector",function(l){function c(a,c){var e={eventName:a.eventName};"undefined"!==typeof a.data&&(e.data=a.data);"undefined"!==typeof a.targetUserId&&(e.targetUserId=a.targetUserId.toString());var d=JSON.stringify(e);if(y)k.debug("send",e,d),y.send(d);else if(c)k.debug("send","The call was ignored because no server connection.",e,d);else throw e=new b.CommonError("send","Trying to call method without established connection","WebSocket is not connected!"),
k.error("send",e),e;}function d(a,c){try{if(a=JSON.parse(a),k.debug("getWebSocketURL",a),a&&a.e&&""!=a.e){var e=new b.CommonError("getWebSocketURL","Error occured while getting the URL of WebSockets",a.e);k.error("getWebSocketURL",e);this.trigger(h.serverError,{error:e})}else{var f=a.d.value;k.info("getWebSocketURL",f);"function"===typeof c&&c(f)}}catch(n){this.ajax(b.ServerConnector.settings.URL,"POST","",s(d,c))}}function f(b,a){y=new WebSocket(b+"/ws/"+encodeURIComponent(a));y.onopen=s(e);y.onclose=
s(p);y.onerror=s(t);y.onmessage=s(n)}function e(b){b={event:b};k.debug("open",b);r&&(u=v.call(this,r));this.trigger(h.connectionOpen,b)}function p(b){u&&(a.clearInterval(u),u=null);b={event:b};k.debug("close",b);this.trigger(h.connectionClose,b);y=null}function t(a){a=new b.CommonError("onerror","WebSocket has got an error",a);k.error("error",a);this.trigger(h.connectionError,{error:a})}function n(b){var a={message:b};k.debug("message",a);this.trigger(h.message,a);if(w(b))if(b=x(b),a=b.type,a==h.usersUpdated||
a==h.userConnected||a==h.userDisconnected)if(a==h.usersUpdated){for(var a=[],c=0,e=b.message.users.length;c<e;c++)a.push({id:b.message.users[c],name:b.message.users[c]});this.trigger(h.usersUpdated,{users:a})}else a==h.userConnected?this.trigger(h.userConnected,{user:{id:b.message,name:b.message}}):a==h.userDisconnected&&this.trigger(h.userDisconnected,{user:{id:b.message,name:b.message}});else if(a==h.receiveOffer||a==h.receiveAnswer||a==h.receiveIce||a==h.receiveBye)a==h.receiveOffer?this.trigger(h.receiveOffer,
{senderId:b.userid,receiverId:b.message.targetUserId,connectionId:b.message.data.connectionId,offer:b.message.data.offer.offer,iceServers:b.message.data.offer.iceServers,connectionType:b.message.data.offer.connectionType,connectionData:b.message.data.offer.connectionData}):a==h.receiveAnswer?this.trigger(h.receiveAnswer,{senderId:b.userid,connectionId:b.message.data.connectionId,answer:b.message.data.answer.answer,acceptData:b.message.data.answer.acceptData}):a==h.receiveIce?this.trigger(h.receiveIce,
{senderId:b.userid,connectionId:b.message.data.connectionId,iceCandidate:b.message.data.iceCandidate}):a==h.receiveBye&&this.trigger(h.receiveBye,{senderId:b.userid,connectionId:b.message.data.connectionId,byeData:b.message.data.byeData})}function w(b){var a=!0;'"Token invalid"'===b.data&&(a=!1,this.trigger(h.tokenInvalid,{token:z}));return a}function x(a){var c;try{c=JSON.parse(a.data)}catch(e){c=null;var d=new b.CommonError("parseServerMessage","Message format error",e);k.error("parseServerMessage",
d,a);this.trigger(h.messageFormatError,{error:d})}return c}function v(b){return a.setInterval(function(){c.call(this,{})},b)}var s=b.Class.proxy(this),k=new b.Logger(this.className),y=null,z=null,r=l?l.pingInterval:5E3,u=null,h=b.ServerConnector.events;b.Class.extend(this,b.EventDispatcher,b.Ajax,{_logger:k,connect:function(a){z=a;a=s(f,a);this.ajax(b.ServerConnector.settings.URL,"POST","",s(d,a))},disconnect:function(){y?(y.close(),z=y=null,k.info("disconnect","Connection with WS has been broken")):
k.debug("disconnect","Connection with WS has not been established yet")},sendOffer:function(b,a,e){c({eventName:h.receiveOffer,targetUserId:b,data:{connectionId:a,offer:e||{}}})},sendAnswer:function(b,a,e){c({eventName:h.receiveAnswer,targetUserId:b,data:{connectionId:a,answer:e||{}}})},sendIce:function(b,a,e){c({eventName:h.receiveIce,targetUserId:b,data:{connectionId:a,iceCandidate:e}})},sendBye:function(b,a,e){c({eventName:h.receiveBye,targetUserId:b,data:{connectionId:a,byeData:e||{}}},!0)}})});
b.ServerConnector.extend({events:{connectionOpen:"connectionopen",connectionClose:"connectionclose",connectionError:"connectionerror",message:"message",messageFormatError:"messageformaterror",serverError:"servererror",tokenInvalid:"tokeninvalid",receiveOffer:"receiveoffer",receiveAnswer:"receiveanswer",receiveIce:"receiveice",receiveBye:"receivebye",usersUpdated:"peers",userConnected:"peer_connected",userDisconnected:"peer_removed"},settings:{URL:"https://api.xirsys.com/wsList"}})})(window);xRtc.stream={};
...
a);N=!0;s.call(this);a=JSON.parse(a.answer);a=new c.RTCSessionDescription(a);g.setRemoteDescription(a);this.trigger(b.Connection.events.answerReceived,{connection:this,user:e,answerData:{answer:a}});this.trigger(b.Connection.events.connectionEstablishing,{connection:this,user:e})}function u(){h.call(this)}function h(){c.detectedBrowser===c.supportedBrowsers.firefox&&O&&(a.clearInterval(O),O=null);if(g){g.onicecandidate=null;g.close();g=null;K=[];E=null;P=N=!1;var d={user:e,connection:this};e=null;
...
name:d};B.connect(a)})},leave:function(){B.on(b.ServerConnector.events.connectionClose,function(){h&&(B.off(r.connectionOpen).off(r.connectionClose).off(r.tokenInvalid).off(r.usersUpdated).off(r.userConnected).off(r.userDisconnected).off(r.receiveOffer).off(r.receiveBye).off(r.receiveAnswer).off(r.receiveIce),h=!1);F={};m=null;q.user=null;u=[];A=[];C={}});B.disconnect()},connect:function(a,c){if(!q.user)throw new b.CommonError("connect","Need to enter the room before you connect someone.");var d=
